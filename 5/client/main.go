...
func getNoteById(baseUrl, id string) string {
	...
	if err == nil && resp.StatusCode == 200 {
		...
		if err == nil {
			ans := note.Name + note.Surname + "написал: " + note.Note
			return ans
		}
	}
...
func sendNote(baseUrl, name, surname, note string) bool {
	values := map[string]string{"name": name, "surname": surname, "note": note}
